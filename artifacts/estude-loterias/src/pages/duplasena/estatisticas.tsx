...
import { useGetDuplasenaEstatisticas } from "@workspace/api-client-react";
...
function DezBadge({ n, active }: { n: number; active?: boolean }) {
  return (
    <span className={cn(
      ...
      active ? "bg-[#a61324] text-white" : "bg-muted text-muted-foreground",
    )}>
...
// ── Main component ────────────────────────────────────────────────────────────
export default function DuplasenaEstatisticas() {
  const [sorteio, setSorteio] = useState<"1" | "2">("1");
  const { data: stats, isLoading, isError } = useGetDuplasenaEstatisticas({ sorteio });
  ...
  if (isError || !stats) {
    return (
      <div className="space-y-6">
        <h1 className="text-3xl font-bold tracking-tight" style={{ color: COR }}>Dupla Sena · Resumo Estatístico</h1>
        <Card>
  ...
  // Dupla Sena: grid 5×10, numbers 01-50
  // Moldura: border of the 5×10 grid (row 1, row 5, col 1 inner, col 10 inner)
  const MOLDURA_SET = new Set([
    ...Array.from({ length: 10 }, (_, i) => i + 1),       // 01-10
    ...Array.from({ length: 10 }, (_, i) => i + 41),       // 41-50
    11, 20, 21, 30, 31, 40,
  ]);
  const RETRATO_SET = new Set(
    Array.from({ length: 50 }, (_, i) => i + 1).filter(n => !MOLDURA_SET.has(n))
  );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card>
            ...
            <CardContent>
                  ...
                  {maisSorteadas.map((item, i) => (
                    <TableRow key={item.dezena} className="odd:bg-muted/40">
                      ...
                      <TableCell className="text-right">
                        {item.ultimoConcurso ? (
                          <Link
                            href={`/duplasena/resultado/${item.ultimoConcurso}`}
                            className="text-xs font-semibold text-[#a61324] hover:underline whitespace-nowrap"
                          >
              ...
              <div className="mt-3 flex justify-end">
                <Link
                  href="/duplasena/tabela-de-dezenas?tab=mais"
                  className="text-sm font-semibold text-[#a61324] hover:underline"
                >
                  Ver todas →
                </Link>
              </div>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="mt-3 flex justify-end">
                <Link
                  href="/duplasena/tabela-de-dezenas?tab=menos"
                  className="text-sm font-semibold text-[#a61324] hover:underline"
                >
                  Ver todas →
                </Link>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="mt-3 flex justify-end">
                <Link
                  href="/duplasena/tabela-de-dezenas?tab=atrasadas"
                  className="text-sm font-semibold text-[#a61324] hover:underline"
                >
                  Ver todas →
                </Link>
                ...
                rows={stats.paresImpares
                  ...
                  .map(d => [
                    ...
                    totalSorteios > 0
                      ...
                      : "–",
                    d.ultimoConcurso
                      ? <Link href={`/duplasena/resultado/${d.ultimoConcurso}`} className="font-semibold text-[#a61324] hover:underline whitespace-nowrap">Concurso {d.ultimoConcurso} →</Link>
                      : "–",
                  ])}
              />
            </CardContent>
              ...
              <CardDescription>
                {stats.somaDezenas.menor && stats.somaDezenas.maior ? (
                  <>
                    ...
                    (
                    <Link
                      href={`/duplasena/resultado/${stats.somaDezenas.menor.concurso}`}
                      className="font-semibold text-[#a61324] hover:underline"
                    >
                    ...
                    (
                    <Link
                      href={`/duplasena/resultado/${stats.somaDezenas.maior.concurso}`}
                      className="font-semibold text-[#a61324] hover:underline"
                    >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription>
                Col. 1 = {"{"}01,11,21,31,41{"}"} … Col. 10 = {"{"}10,20,30,40,50{"}"}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="h-[220px]">
                  ...
                  >
                    ...
                    <Tooltip
                      ...
                      content={({ active, payload }) => {
                        ...
                        const dezenas = [base, base + 10, base + 20, base + 30, base + 40]
                          .filter(n => n <= 50)
                          .map(n => String(n).padStart(2, "0"))
                          .join(", ");
                        return (
            ...
            <CardContent className="pt-5 space-y-5">
              <div className="flex flex-wrap gap-x-8 gap-y-1 text-sm">
                <div>
                  ...
                  <span className="font-semibold">{especial.quantidadeNaFaixa} dezenas entre 01 e 50</span>
                </div>
                <div>
