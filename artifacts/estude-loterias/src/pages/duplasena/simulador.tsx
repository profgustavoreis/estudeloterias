...
import { useSimularDuplasena, SimulacaoResultadoDuplasena } from "@workspace/api-client-react";
...
function probDuplasena(k: number, N: number): string {
  const total = comb(50, 6);
  const ways = comb(N, k) * comb(50 - N, 6 - k);
  if (ways <= 0) return "—";
  const x = total / ways;
...
type Filtro = "todos" | "premiados" | "sena" | "quina" | "quadra" | "terno";

const FILTRO_LABELS: Record<Filtro, string> = {
  todos:       "Todos os concursos",
  premiados:   "Somente concursos premiados (6, 5, 4 ou 3 acertos)",
  sena:        "Somente concursos com 6 acertos (sena)",
...
export default function DuplasenaSimulador() {
  const [selecionadas, setSelecionadas] = useState<Set<number>>(new Set());
  const [filtro, setFiltro] = useState<Filtro>("premiados");
  const [resultado, setResultado] = useState<SimulacaoResultadoDuplasena | null>(null);
  const [nSimulado, setNSimulado] = useState<number>(6);
  const [infoAberta, setInfoAberta] = useState(false);
  const [paginaTabela, setPaginaTabela] = useState(1);

  const simular = useSimularDuplasena({ mutation: { onSuccess: data => { setResultado(data); setNSimulado(selecionadas.size); setPaginaTabela(1); } } });
  ...
  const totalConcursosArray = resultado?.concursos.length ?? 0;
  const totalPaginasTabela = Math.max(1, Math.ceil(totalConcursosArray / PAGE_SIZE));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight" style={{ color: COR }}>Dupla Sena · Simulador Histórico</h1>
          <p className="text-muted-foreground mt-1">Selecione de 6 a 15 dezenas e descubra em quantos sorteios anteriores você teria acertado.</p>
        </div>
      </div>
          ...
          <Card>
            <CardContent className="pt-5 space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">Na Dupla Sena, cada concurso possui dois sorteios. São premiadas apostas com 6, 5, 4 ou 3 acertos.</p>
              </div>
          ...
          {resultado && (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-base font-semibold">Resultado da Simulação</span>
                <span className="text-sm text-muted-foreground ml-auto">
                  {nSimulado} dezena{nSimulado > 1 ? "s" : ""} • {resultado.totalConcursos.toLocaleString("pt-BR")} concursos •{" "}
              ...
              <div className="flex flex-col gap-4">
                <Card className="border-t-4" style={{ borderTopColor: COR }}>
                  <CardHeader className="pb-2">
                    <div className="flex items-center gap-2">
                      <span className="inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold text-white" style={{ backgroundColor: COR }}>1</span>
                      <CardTitle className="text-sm">1º Sorteio</CardTitle>
                    </div>
                  </CardHeader>
                  <CardContent className="pb-3">
                    <Table>
                      <TableHeader>
                        <TableRow>
                          <TableHead className="text-center w-18">Acertos</TableHead>
                          <TableHead className="text-center">Probabilidade</TableHead>
                          <TableHead className="text-center w-20">Ocorrências</TableHead>
                          <TableHead className="text-center w-16">Situação</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {resumoLinhas1.map(r => {
                          const premiado = FAIXAS_PREMIADAS.has(r.acertos);
                          const prob = probDuplasena(r.acertos, nSimulado);
                          return (
                            <TableRow key={`1-${r.acertos}`} className={cn(premiado && "font-semibold")} style={premiado ? { backgroundColor: COR + "0d" } : {}}>
                ...
                <Card className="border-t-4" style={{ borderTopColor: COR }}>
                  <CardHeader className="pb-2">
                    <div className="flex items-center gap-2">
                      <span className="inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold text-white" style={{ backgroundColor: "#d97706" }}>2</span>
                      <CardTitle className="text-sm">2º Sorteio</CardTitle>
                    </div>
                  </CardHeader>
                  <CardContent className="pb-3">
                    <Table>
                      ...
                      <TableBody>
                        {resumoLinhas2.map(r => {
                          ...
                          const prob = probDuplasena(r.acertos, nSimulado);
                          return (
                            <TableRow key={`2-${r.acertos}`} className={cn(premiado && "font-semibold")} style={premiado ? { backgroundColor: COR + "0d" } : {}}>
                              <TableCell className="text-center font-bold">{r.acertos}</TableCell>
                              <TableCell className="text-center text-sm tabular-nums text-muted-foreground">{prob}</TableCell>
                              <TableCell className="text-center">{r.contagem.toLocaleString("pt-BR")}</TableCell>
                              <TableCell className="text-center">{premiado ? <span>{PREMIADO_EMOJI}</span> : <span className="text-muted-foreground text-xs">—</span>}</TableCell>
                      ...
                      <Table className="w-full">
                        <TableHeader>
                          <TableRow>
                            ...
                            <TableHead className="text-center whitespace-nowrap">Sorteio</TableHead>
                        ...
                        <TableBody>
                          {concursosPagina.map(c => {
                            ...
                            return (
                              <TableRow key={`${c.concurso}-${c.sorteio}`}>
                                ...
                                <TableCell className="text-center whitespace-nowrap">
                                  <span className={cn("inline-flex items-center justify-center w-6 h-6 rounded-full text-[10px] font-bold",
                                    c.sorteio === 1 ? "text-white" : "text-amber-800")}
                                    style={c.sorteio === 1 ? { backgroundColor: COR } : { backgroundColor: "#fde68a" }}>
                                    {c.sorteio}º
                                  </span>
                                </TableCell>
                                ...
                                <TableCell className="text-right pr-4 whitespace-nowrap">
                                  <Link href={`/duplasena/resultado/${c.concurso}`} className="text-sm font-semibold hover:underline" style={{ color: COR }}>Ver detalhes →</Link>
                                </TableCell>
                              </TableRow>
                            );
                          })}
