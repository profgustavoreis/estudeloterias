...
import { useGerarJogoDuplasena } from "@workspace/api-client-react";
import { Sparkles, Printer, RefreshCw } from "lucide-react";
import { PageSEO } from "@/components/seo/PageSEO";

const COR = "#a61324";
const COR_HOVER = "#87121c";
const PRECO_APOSTA = 3.0;

export default function DuplasenaGerador() {
  ...
  const gerarJogoDuplasena = useGerarJogoDuplasena({
    mutation: { onSuccess: data => setResultado(data) },
  });

  const handleGerar = () => {
    gerarJogoDuplasena.mutate({ data: { quantidadeJogos: qtdJogos, quantidadeDezenas: qtdDezenas } });
  };

  return (
    <div className="space-y-6">
      <PageSEO
        title="Gerador de Jogos da Dupla Sena"
        description="Gere apostas aleatórias para a Dupla Sena. Surpresinha inteligente e gratuita: cada jogo tem de 6 a 15 dezenas de 01 a 50."
        canonical="/duplasena/gerador"
      />
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight" style={{ color: COR }}>Dupla Sena · Gerador de Jogos</h1>
          <p className="text-muted-foreground mt-1">Crie jogos aleatórios de 6 a 15 dezenas para a Dupla Sena.</p>
        </div>
      </div>
        ...
        <Card className="lg:col-span-1 h-fit">
          ...
          <CardContent className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              <Slider value={[qtdDezenas]} min={6} max={15} step={1} onValueChange={v => setQtdDezenas(v[0])} />
              <div className="text-xs text-muted-foreground">Aposta mínima: 6 dezenas (R$ 3,00). Máxima: 15 dezenas.</div>
            </div>

            <div className="p-4 bg-muted/50 rounded-lg border border-border space-y-1">
              <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Aposta da Dupla Sena</div>
              <p className="text-sm text-muted-foreground">
                Cada jogo é uma aposta de <strong>{qtdDezenas} dezenas</strong> (01 a 50).
              </p>
              <p className="text-sm text-muted-foreground">
                    ...
                    <CardContent className="p-4">
                      <div className="flex flex-wrap gap-2">
                        {jogo.sort((a, b) => a - b).map(num => (
                          <LotteryBall key={num} number={num} size="md" color={COR} />
                        ))}
                      </div>
                    </CardContent>
                  </Card>
