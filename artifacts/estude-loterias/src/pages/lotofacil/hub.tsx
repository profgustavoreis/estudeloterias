import { useGetLotofacilUltimoResultado } from "@workspace/api-client-react";
...
export default function LotofacilHub() {
  const { data: resultado, isLoading } = useGetLotofacilUltimoResultado();

  const quickLinks = [
    { href: `/lotofacil/resultado/${resultado?.concurso ?? ""}`, label: "Último Resultado", icon: Dices, desc: "Detalhes do último sorteio" },
    { href: "/lotofacil/resultados", label: "Resultados Anteriores", icon: List, desc: "Busque por sorteios passados" },
    { href: "/lotofacil/resumo-estatistico", label: "Resumo Estatístico", icon: BarChart3, desc: "Frequência de dezenas e atrasos" },
    { href: "/lotofacil/tabela-de-dezenas", label: "Tabela de Dezenas", icon: TableIcon, desc: "Ranking detalhado de todas as dezenas" },
    { href: "/lotofacil/gerador", label: "Gerador de Jogos", icon: Sparkles, desc: "Gere jogos inteligentes" },
    { href: "/lotofacil/simulador", label: "Simulador Histórico", icon: FlaskConical, desc: "Confira sua aposta no histórico" },
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight" style={{ color: COR }}>Lotofácil · Painel Principal</h1>
            <p className="text-muted-foreground mt-1">A loteria com mais sorteios semanais do Brasil.</p>
          </div>
        </div>
        <ConcursoNavigator
        ...
        <Card className="lg:col-span-2 border-t-4" style={{ borderTopColor: COR }}>
          <CardHeader>
            <div className="flex justify-between items-start">
              ...
              <Link href={`/lotofacil/resultado/${resultado.concurso}`}>
