...
import {
  useGetLotofacilUltimoResultado,
  useGetLotofacilResultadoConcurso,
...
function DezenasCard({ resultado }: { resultado: ResultadoLotofacil }) {
  return (
...
function RepeticoesCard({ concurso, dezenas }: { concurso: number; dezenas: string[] }) {
  const { data: prev } = useGetLotofacilResultadoConcurso(concurso - 1);
...
function RateioCard({ resultado }: { resultado: ResultadoLotofacil }) {
  return (
    <Card>
...
function ResultadoView({ resultado, latestConcurso }: { resultado: ResultadoLotofacil; latestConcurso: number }) {
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-3xl font-bold tracking-tight" style={{ color: BRAND }}>
            Lotofácil · Concurso {resultado.concurso}
          </h1>
...
function LatestResultado() {
  const { data, isLoading, isError } = useGetLotofacilUltimoResultado();
...
function ConcursoResultado({ concurso }: { concurso: number }) {
  const { data, isLoading, isError } = useGetLotofacilResultadoConcurso(concurso);
  const { data: latest } = useGetLotofacilUltimoResultado();
  if (isLoading) return <LoadingSkeleton />;
...
export default function LotofacilUltimoResultado() {
