...
export default function LotofacilResultadosAnteriores() {
  const [page, setPage] = useState(1);
  const [ano, setAno] = useState<number | null>(null);
  const [ordem, setOrdem] = useState<"asc" | "desc">("desc");

  const { data, isLoading, isError } = useGetLotofacilResultados({ page, limit: 20, ano, ordem });

  const years = Array.from({ length: new Date().getFullYear() - 2002 }, (_, i) => new Date().getFullYear() - i);

  return (
    <div className="space-y-6">
      <PageSEO
        title="Todos os Resultados da Lotofácil"
        description="Consulte o histórico completo de todos os resultados da Lotofácil. Filtre por ano, veja dezenas sorteadas e prêmios de cada concurso."
        canonical="/lotofacil/resultados"
      />
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight" style={{ color: COR }}>Lotofácil · Resultados Anteriores</h1>
            <p className="text-muted-foreground mt-1">Busque e analise os concursos passados da Lotofácil.</p>
          </div>
        </div>
                  ...
                  data.resultados.map(res => (
                    <TableRow key={res.concurso}>
                      ...
                      <TableCell className="text-right">
                        <Link href={`/lotofacil/resultado/${res.concurso}`} className="text-sm font-semibold hover:underline whitespace-nowrap" style={{ color: COR }}>
                          Ver detalhes →
                        </Link>
                      </TableCell>
                    </TableRow>
