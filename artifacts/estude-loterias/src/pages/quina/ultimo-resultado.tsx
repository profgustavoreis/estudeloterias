...
import {
  useGetQuinaUltimoResultado,
  useGetQuinaResultadoConcurso,
...
function DezenasCard({ resultado }: { resultado: ResultadoQuina }) {
  return (
...
function RepeticoesCard({ concurso, dezenas }: { concurso: number; dezenas: string[] }) {
  const { data: prev } = useGetQuinaResultadoConcurso(concurso - 1);
...
function RateioCard({ resultado }: { resultado: ResultadoQuina }) {
  return (
    <Card>
...
function ResultadoView({ resultado, latestConcurso }: { resultado: ResultadoQuina; latestConcurso: number }) {
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-3xl font-bold tracking-tight" style={{ color: BRAND }}>
            Quina · Concurso {resultado.concurso}
          </h1>
          <p className="text-muted-foreground mt-1 text-sm">Sorteio realizado dia {formatDateShort(resultado.data)}</p>
        </div>
        <ConcursoNavigator concurso={resultado.concurso} isLatest={isLatest} latestConcurso={latestConcurso} basePath="/quina/resultado" color={BRAND} />
      </div>
...
function LatestResultado() {
  const { data, isLoading, isError } = useGetQuinaUltimoResultado();
...
function ConcursoResultado({ concurso }: { concurso: number }) {
  const { data, isLoading, isError } = useGetQuinaResultadoConcurso(concurso);
  const { data: latest } = useGetQuinaUltimoResultado();
  if (isLoading) return <LoadingSkeleton />;
...
export default function QuinaUltimoResultado() {
