...
export default function MaismilionariaSimulador() {
  ...
  const resumoLinhas = resultado?.resumo.filter(r => r.acertos >= 0 && r.acertos <= 6) ?? [];

  return (
    <div className="space-y-6">
      <PageSEO
        title="Simulador Histórico da +Milionária — Teste sua Aposta no Histórico"
        description="Escolha suas 6 dezenas e descubra em quantos sorteios da +Milionária você teria ganhado. Simulador histórico gratuito e completo."
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">Selecione exatamente 6 dezenas e descubra em quantos sorteios anteriores você teria acertado.</p>
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          <Card>
            <CardContent className="pt-5 space-y-4">
              ...
              {count === 0 && <p className="text-sm text-muted-foreground text-center">Selecione 6 números e clique em Simular.</p>}
            </CardContent>
          </Card>
          ...
          {resultado && (
            <Card className="border-t-4" style={{ borderTopColor: COR }}>
              <CardHeader className="pb-3">
                ...
                <CardDescription>
                  6 dezenas escolhidas • {resultado.totalConcursos.toLocaleString("pt-BR")} concursos •{" "}
              ...
              <CardContent className="pb-3">
                      ...
                      return (
                        <TableRow key={r.acertos} className={cn(premiado && "font-semibold")} style={premiado ? { backgroundColor: COR + "0d" } : {}}>
                          ...
                          <TableCell className="text-center text-sm tabular-nums text-muted-foreground">
                            {r.acertos === 6 ? "1 em 158.145.470" : r.acertos === 5 ? "1 em 690.136" : r.acertos === 4 ? "1 em 8.855" : r.acertos === 3 ? "1 em 256" : "—"}
                          </TableCell>
                ...
                <p className="mt-3 text-xs text-muted-foreground leading-snug border-t pt-3">As probabilidades indicadas correspondem a uma aposta de 6 dezenas na +Milionária.</p>
              </CardContent>
            </Card>
          )}
        </div>
