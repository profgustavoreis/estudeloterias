...
import { useGetTimemaniaResultados } from "@workspace/api-client-react";
...
export default function TimemaniaResultadosAnteriores() {
  const [page, setPage] = useState(1);
  const [ano, setAno] = useState<number | null>(null);
  const [ordem, setOrdem] = useState<"asc" | "desc">("desc");

  const { data, isLoading, isError } = useGetTimemaniaResultados({ page, limit: 20, ano, ordem });

  const years = Array.from({ length: new Date().getFullYear() - 2007 }, (_, i) => new Date().getFullYear() - i);

  return (
    <div className="space-y-6">
      <PageSEO
        title="Todos os Resultados da Timemania"
        description="Consulte o histórico completo de todos os resultados da Timemania. Filtre por ano, veja dezenas sorteadas e prêmios de cada concurso."
        canonical="/timemania/resultados"
      />
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight" style={{ color: COR }}>Timemania · Resultados Anteriores</h1>
            <p className="text-muted-foreground mt-1">Busque e analise os concursos passados da Timemania.</p>
          </div>
        </div>
                ...
                {isLoading ? (
                  ...
                  data.resultados.map(res => (
                    <TableRow key={res.concurso}>
                      ...
                      <TableCell>
                        <Link href={`/timemania/resultado/${res.concurso}`}>
