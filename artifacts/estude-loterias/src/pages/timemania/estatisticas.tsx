...
export default function TimemaniaEstatisticas() {
  ...
  // Grid 8×10 — moldura/retrato for Timemania (80 numbers)
  ...
  const RETRATO_SET = new Set(
    Array.from({ length: 80 }, (_, i) => i + 1).filter(n => !MOLDURA_SET.has(n))
  );
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card>
            ...
            <CardContent>
                  ...
                  {maisSorteadas.map((item, i) => (
                    <TableRow key={item.dezena} className="odd:bg-muted/40">
                      ...
                      <TableCell className="text-right">
                        {item.ultimoConcurso ? (
                          <Link href={`/timemania/resultado/${item.ultimoConcurso}`} className="text-xs font-semibold hover:underline whitespace-nowrap" style={{ color: COR }}>
              ...
              <div className="mt-3 flex justify-end">
                <Link href="/timemania/tabela-de-dezenas?tab=mais" className="text-sm font-semibold hover:underline" style={{ color: COR }}>Ver todas →</Link>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
                ...
                <TableBody>
                  {menosSorteadas.map((item, i) => (
                    <TableRow key={item.dezena} className="odd:bg-muted/40">
                      ...
                      <TableCell className="text-right">
                        {item.ultimoConcurso ? (
                          <Link href={`/timemania/resultado/${item.ultimoConcurso}`} className="text-xs font-semibold hover:underline whitespace-nowrap" style={{ color: COR }}>Concurso {item.ultimoConcurso} →</Link>
                        ) : <span className="text-xs text-muted-foreground">–</span>}
                      </TableCell>
                    </TableRow>
                  ))}
              ...
              <div className="mt-3 flex justify-end">
                <Link href="/timemania/tabela-de-dezenas?tab=menos" className="text-sm font-semibold hover:underline" style={{ color: COR }}>Ver todas →</Link>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="mt-3 flex justify-end">
                <Link href="/timemania/tabela-de-dezenas?tab=atrasadas" className="text-sm font-semibold hover:underline" style={{ color: COR }}>Ver todas →</Link>
              </div>
            </CardContent>
          </Card>
        </div>
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription>Como dezenas pares e ímpares se distribuem ao longo dos sorteios?</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="h-[200px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={stats.paresImpares} margin={{ top: 22, right: 8, left: -20, bottom: 0 }}>
                    <XAxis dataKey="pares" tickLine={false} axisLine={false} tickFormatter={(v) => `${v}P/${7 - v}I`} tick={{ fontSize: 11 }} />
                    ...
                    <Tooltip cursor={{ fill: "rgba(0,0,0,0.04)" }} content={({ active, payload }) => {
                      ...
                      return (
                        <div className="bg-card border rounded shadow-md px-3 py-2 text-sm">
                          <p className="font-semibold mb-0.5">{d.pares} par{d.pares !== 1 ? "es" : ""} / {d.impares} impar{d.impares !== 1 ? "es" : ""}</p>
                          <p className="text-muted-foreground">{d.sorteios.toLocaleString("pt-BR")} concursos</p>
                          {totalSorteios > 0 && <p className="text-muted-foreground">{((d.sorteios / totalSorteios) * 100).toFixed(1)}% do total</p>}
                        </div>
                      );
                    }} />
              ...
              <CompactTable
                ...
                rows={stats.paresImpares.filter(d => d.sorteios > 0).map(d => [
                  d.pares, d.impares, d.sorteios.toLocaleString("pt-BR"),
                  totalSorteios > 0 ? `${((d.sorteios / totalSorteios) * 100).toFixed(1)}%` : "–",
                  d.ultimoConcurso ? <Link href={`/timemania/resultado/${d.ultimoConcurso}`} className="font-semibold hover:underline whitespace-nowrap" style={{ color: COR }}>Concurso {d.ultimoConcurso} →</Link> : "–",
                ])}
              />
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="h-[200px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={stats.molduraRetrato ?? []} margin={{ top: 22, right: 8, left: -20, bottom: 0 }}>
                    <XAxis dataKey="moldura" tickLine={false} axisLine={false} tickFormatter={(v) => `${v}M/${7 - v}R`} tick={{ fontSize: 11 }} />
                    <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 11 }} />
                    <Tooltip cursor={{ fill: "rgba(0,0,0,0.04)" }} content={({ active, payload }) => {
                      if (!active || !payload?.length) return null;
                      ...
                      return (
                        <div className="bg-card border rounded shadow-md px-3 py-2 text-sm">
                          <p className="font-semibold mb-0.5">{d.moldura} moldura / {d.retrato} retrato</p>
                          <p className="text-muted-foreground">{d.sorteios.toLocaleString("pt-BR")} concursos</p>
                          {totalMoldura > 0 && <p className="text-muted-foreground">{((d.sorteios / totalMoldura) * 100).toFixed(1)}% do total</p>}
                        </div>
                      );
                    }} />
                    <Bar dataKey="sorteios" fill={COR} radius={[4, 4, 0, 0]}>
              ...
              <CompactTable
                ...
                rows={(stats.molduraRetrato ?? []).filter(d => d.sorteios > 0).map(d => [
                  d.moldura, d.retrato, d.sorteios.toLocaleString("pt-BR"),
                  totalMoldura > 0 ? `${((d.sorteios / totalMoldura) * 100).toFixed(1)}%` : "–",
                  d.ultimoConcurso ? <Link href={`/timemania/resultado/${d.ultimoConcurso}`} className="font-semibold hover:underline whitespace-nowrap" style={{ color: COR }}>Concurso {d.ultimoConcurso} →</Link> : "–",
                ])}
              />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription>
                {stats.somaDezenas.menor && stats.somaDezenas.maior ? (
                  <>Menor: {stats.somaDezenas.menor.valor} (<Link href={`/timemania/resultado/${stats.somaDezenas.menor.concurso}`} className="font-semibold hover:underline" style={{ color: COR }}>Concurso {stats.somaDezenas.menor.concurso} →</Link>) • Maior: {stats.somaDezenas.maior.valor} (<Link href={`/timemania/resultado/${stats.somaDezenas.maior.concurso}`} className="font-semibold hover:underline" style={{ color: COR }}>Concurso {stats.somaDezenas.maior.concurso} →</Link>)</>
            ...
            <CardContent>
              <div className="h-[260px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={stats.somaDezenas.intervalos.map((d) => ({ ...d, _display: Math.max(d.sorteios, 1) }))} layout="vertical" margin={{ top: 4, right: 52, left: 4, bottom: 0 }}>
                    <XAxis type="number" tickLine={false} axisLine={false} tick={{ fontSize: 11 }} />
                    <YAxis type="category" dataKey="faixa" tickLine={false} axisLine={false} tick={{ fontSize: 11 }} width={72} />
                    <Tooltip cursor={{ fill: "rgba(0,0,0,0.04)" }} content={({ active, payload }) => {
                      if (!active || !payload?.length) return null;
                      ...
                      return (
                        <div className="bg-card border rounded shadow-md px-3 py-2 text-sm">
                          ...
                          <p className="text-muted-foreground">{d.sorteios.toLocaleString("pt-BR")} concursos</p>
                          {totalSomaConcursos > 0 && <p className="text-muted-foreground">{((d.sorteios / totalSomaConcursos) * 100).toFixed(1)}%</p>}
                        </div>
                      );
                    }} />
              ...
              <CompactTable
                ...
                rows={stats.somaDezenas.intervalos.map(d => [
                  d.faixa.replace("\u2013", " a ").replace("-", " a "), d.sorteios.toLocaleString("pt-BR"),
                  totalSomaConcursos > 0 ? `${((d.sorteios / totalSomaConcursos) * 100).toFixed(1)}%` : "–",
                  d.ultimoConcurso ? <Link href={`/timemania/resultado/${d.ultimoConcurso}`} className="font-semibold hover:underline whitespace-nowrap" style={{ color: COR }}>Concurso {d.ultimoConcurso} →</Link> : "–",
                ])}
              />
            </CardContent>
          ...
          <div className="flex flex-col gap-4">
            <AdUnit slot="6677889911" format="rectangle" className="w-full" />
          </div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="h-[220px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={stats.frequenciaPorLinha} margin={{ top: 22, right: 8, left: -20, bottom: 0 }}>
                    <XAxis dataKey="faixa" tickLine={false} axisLine={false} tick={{ fontSize: 11 }} />
                    <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 11 }} />
                    <Tooltip cursor={{ fill: "rgba(0,0,0,0.04)" }} content={({ active, payload }) => {
                      if (!active || !payload?.length) return null;
                      const d = payload[0].payload as { faixa: string; sorteios: number };
                      return (<div className="bg-card border rounded shadow-md px-3 py-2 text-sm"><p className="font-semibold mb-0.5">Linha {d.faixa}</p><p className="text-muted-foreground">{d.sorteios.toLocaleString("pt-BR")} ocorrências</p></div>);
                    }} />
              ...
              <CompactTable
                ...
                rows={stats.frequenciaPorLinha.map((d, i) => [
                  `Linha ${i + 1}`, d.sorteios.toLocaleString("pt-BR"),
                  totalFreqLinha > 0 ? `${((d.sorteios / totalFreqLinha) * 100).toFixed(1)}%` : "–",
                ])}
              />
              <Accordion type="single" collapsible className="mt-2 border rounded-md px-3 bg-muted/20">
                    ...
                    <div className="space-y-1 text-xs text-muted-foreground">
                      {Array.from({ length: 8 }, (_, i) => (
                        <div key={i}>
                          ...
                          {Array.from({ length: 10 }, (_, j) => i * 10 + j + 1).map(n => String(n).padStart(2, "0")).join(", ")}
                        </div>
                      ))}
                    </div>
                  </AccordionContent>
          ...
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription>
                Col. 1 = {"{"}01,11,21,31,41,51,61,71{"}"} … Col. 10 = {"{"}10,20,30,40,50,60,70,80{"}"}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="h-[220px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={stats.frequenciaPorColuna} margin={{ top: 22, right: 8, left: -20, bottom: 0 }}>
                    <XAxis dataKey="coluna" tickLine={false} axisLine={false} tick={{ fontSize: 11 }} tickFormatter={(v) => `C${v}`} />
                    <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 11 }} />
                    <Tooltip cursor={{ fill: "rgba(0,0,0,0.04)" }} content={({ active, payload }) => {
                      if (!active || !payload?.length) return null;
                      const d = payload[0].payload as { coluna: number; sorteios: number };
                      const base = d.coluna;
                      const dezenas = [base, base + 10, base + 20, base + 30, base + 40, base + 50, base + 60, base + 70]
                        .filter(n => n <= 80).map(n => String(n).padStart(2, "0")).join(", ");
                      return (<div className="bg-card border rounded shadow-md px-3 py-2 text-sm"><p className="font-semibold mb-0.5">Coluna {d.coluna}</p><p className="text-muted-foreground text-xs mb-0.5">{dezenas}</p><p className="text-muted-foreground">{d.sorteios.toLocaleString("pt-BR")} ocorrências</p></div>);
                    }} />
                    <Bar dataKey="sorteios" fill={COR} radius={[3, 3, 0, 0]}>
                      <LabelList dataKey="sorteios" position="top" style={{ fontSize: 13, fontWeight: "bold", fill: "#333" }} formatter={(v: number) => v.toLocaleString("pt-BR")} />
                    </Bar>
              ...
              <CompactTable
                ...
                rows={stats.frequenciaPorColuna.map(d => [
                  `Coluna ${d.coluna}`, d.sorteios.toLocaleString("pt-BR"),
                  totalFreqColuna > 0 ? `${((d.sorteios / totalFreqColuna) * 100).toFixed(1)}%` : "–",
                ])}
              />
              <Accordion type="single" collapsible className="mt-2 border rounded-md px-3 bg-muted/20">
                  ...
                  <AccordionContent>
                    <div className="space-y-1 text-xs text-muted-foreground">
                      {Array.from({ length: 10 }, (_, i) => i + 1).map(col => {
                        ...
                        const dezenas = [base, base + 10, base + 20, base + 30, base + 40, base + 50, base + 60, base + 70]
                          .filter(n => n <= 80).map(n => String(n).padStart(2, "0")).join(", ");
                        return (<div key={col}><span className="font-semibold text-foreground">Coluna {col}: </span>{dezenas}</div>);
                      })}
                    </div>
                  </AccordionContent>
                </AccordionItem>
      ...
      <section className="space-y-3">
        <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Números Especiais</h2>
              ...
              <CardHeader className="pb-0">
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div>
                    ...
                    <CardDescription className="mt-1">E mais uma vez fica provado que a Matemática é a melhor de todas!</CardDescription>
                  </div>
                  <div className="flex gap-1 bg-muted rounded-lg p-1">
                    {stats.numerosEspeciais.map((item, i) => (
                      <button key={item.tipo} onClick={() => setTabEspecial(i)} className={cn("px-3 py-1.5 text-sm font-medium rounded-md transition-colors", tabEspecial === i ? "bg-background shadow text-foreground" : "text-muted-foreground hover:text-foreground")}>
                        {item.label}
                      </button>
                    ))}
                  </div>
              ...
              {especial && (
                <CardContent className="pt-5 space-y-5">
                  <div className="flex flex-wrap gap-x-8 gap-y-1 text-sm">
                    <div><span className="text-muted-foreground">Total: </span><span className="font-semibold">{especial.quantidadeNaFaixa} dezenas entre 01 e 80</span></div>
                    <div><span className="text-muted-foreground">Média de ocorrências por sorteio: </span><span className="font-semibold">{especial.media.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></div>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {Array.from({ length: 80 }, (_, i) => i + 1).map(n => (<DezBadge key={n} n={n} active={especialSet.has(n)} />))}
                  </div>
                  {(() => {
                    const titleMap: Record<string, string> = { primos: "Distribuição da quantidade de números primos por sorteio", fibonacci: "Distribuição da quantidade de números de Fibonacci por sorteio", triangulares: "Distribuição da quantidade de números triangulares por sorteio" };
                    const totalEspecial = especial.distribuicao.reduce((a, d) => a + d.sorteios, 0);
                    return (
                      <div>
                        <p className="text-xs font-medium text-muted-foreground mb-2">{titleMap[especial.tipo] ?? especial.label}</p>
                        <div className="h-[180px]">
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={especial.distribuicao.filter(d => d.sorteios > 0)} margin={{ top: 22, right: 8, left: -20, bottom: 0 }}>
                              <XAxis dataKey="count" tickLine={false} axisLine={false} tick={{ fontSize: 11 }} tickFormatter={(v) => `${v}`} />
                              <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 11 }} />
                              <Tooltip cursor={{ fill: "rgba(0,0,0,0.04)" }} content={({ active, payload }) => {
                                if (!active || !payload?.length) return null;
                                const d = payload[0].payload as { count: number; sorteios: number };
                                return (<div className="bg-card border rounded shadow-md px-3 py-2 text-sm"><p className="font-semibold mb-0.5">{d.count} {especial.label.toLowerCase()} no sorteio</p><p className="text-muted-foreground">{d.sorteios.toLocaleString("pt-BR")} concursos</p></div>);
                              }} />
                              <Bar dataKey="sorteios" fill={COR} radius={[4, 4, 0, 0]}><LabelList dataKey="sorteios" position="top" style={{ fontSize: 13, fontWeight: "bold", fill: "#333" }} formatter={(v: number) => v.toLocaleString("pt-BR")} /></Bar>
                            </BarChart>
                          </ResponsiveContainer>
                        </div>
                        <CompactTable headers={[especial.label, "Concursos", "%", "Última vez"]} rows={especial.distribuicao.map(d => [d.count, d.sorteios.toLocaleString("pt-BR"), totalEspecial > 0 ? `${((d.sorteios / totalEspecial) * 100).toFixed(1)}%` : "–", d.ultimoConcurso ? <Link href={`/timemania/resultado/${d.ultimoConcurso}`} className="font-semibold hover:underline whitespace-nowrap" style={{ color: COR }}>Concurso {d.ultimoConcurso} →</Link> : "–"])} />
                      </div>
                    );
                  })()}
                </CardContent>
