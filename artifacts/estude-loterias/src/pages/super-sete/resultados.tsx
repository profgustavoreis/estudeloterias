...
import { useGetSuperSeteResultados } from "@workspace/api-client-react";
...
export default function SuperSeteResultadosAnteriores() {
  const [page, setPage] = useState(1);
  const [ano, setAno] = useState<number | null>(null);
  const [ordem, setOrdem] = useState<"asc" | "desc">("desc");

  const { data, isLoading, isError } = useGetSuperSeteResultados({ page, limit: 20, ano, ordem });

  const years = Array.from({ length: new Date().getFullYear() - 2021 + 1 }, (_, i) => new Date().getFullYear() - i);

  return (
    <div className="space-y-6">
      <PageSEO
        title="Todos os Resultados da Super Sete"
        description="Consulte o histórico completo de todos os resultados da Super Sete. Filtre por ano, veja a sequência de 7 números e prêmios de cada concurso."
        canonical="/super-sete/resultados"
      />
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight" style={{ color: COR }}>Super Sete · Resultados Anteriores</h1>
            <p className="text-muted-foreground mt-1">Busque e analise os concursos passados da Super Sete.</p>
          </div>
        </div>
                    ...
                    <TableRow key={res.concurso}>
                      ...
                      <TableCell>
                        <div className="flex justify-center gap-1 flex-wrap">
                          {res.dezenas.map((d, i) => (
                            <LotteryBall key={i} number={d} size="sm" color={BALL_BG} textColor={BALL_TEXT} padDigits={1} />
                          ))}
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        <Link href={`/super-sete/resultado/${res.concurso}`}>
      ...
      {data && data.totalPaginas > 1 && (
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm text-muted-foreground">
            Página {data.pagina} de {data.totalPaginas} ({data.total} resultados)
          </span>
