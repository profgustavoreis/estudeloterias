import { useGetSuperSeteUltimoResultado } from "@workspace/api-client-react";
...
export default function SuperSeteHub() {
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight" style={{ color: COR }}>Super Sete · Painel Principal</h1>
            <p className="text-muted-foreground mt-1">Escolha um número de 0 a 9 em cada uma das 7 colunas e concorra acertando a sequência sorteada!</p>
          </div>
        </div>
        <ConcursoNavigator
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex justify-center flex-wrap gap-3 py-6 bg-muted/30 rounded-xl border border-border/50">
              {resultado.dezenas.map((num, i) => (
                <LotteryBall key={i} number={num} size="lg" color={BALL_BG} textColor={BALL_TEXT} padDigits={1} />
              ))}
            </div>
...
Na Super Sete você escolhe um número de 0 a 9 em cada uma das 7 colunas, formando uma sequência de 7 números.
                 São sorteados 7 números por concurso, um por coluna (0-9). Você ganha acertando 7, 6, 5, 4 ou 3 números
                 na posição correta. É possível apostar 2 ou 3 números por coluna (aposta múltipla).
                A Super Sete não possui sorteio especial.
              </p>
            </div>
          </CardContent>
        </Card>
