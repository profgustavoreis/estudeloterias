...
import { useGetLotomaniaEstatisticas } from "@workspace/api-client-react";
...
// Lotomania 10×10 grid definitions
// O volante segue a sequência visual: 01, 02, ..., 99, 00 (última posição).
// Moldura = borda externa: linha 1 (01–10), linha 10 (91–99, 00), colunas laterais das linhas 2–9.
// Usamos valores numéricos onde "00" = 0.
const MOLDURA_SET = new Set<number>([
  ...Array.from({ length: 10 }, (_, i) => i + 1),    // linha 1: 01–10
  ...Array.from({ length: 9 }, (_, i) => i + 91),    // linha 10: 91–99
  0,                                                  // "00" (última posição, linha 10)
  11, 21, 31, 41, 51, 61, 71, 81,                    // coluna esquerda das linhas 2–9
  20, 30, 40, 50, 60, 70, 80, 90,                    // coluna direita das linhas 2–9
]);
const RETRATO_SET = new Set(
  Array.from({ length: 100 }, (_, i) => i).filter(n => !MOLDURA_SET.has(n))
);
// Sequência visual do volante para exibição: 01, 02, ..., 99, 00
const VOLANTE_SEQ = [...Array.from({ length: 99 }, (_, i) => i + 1), 0];
...
// ── Main component ────────────────────────────────────────────────────────────
export default function LotomaniaEstatisticas() {
  const { data: stats, isLoading, isError } = useGetLotomaniaEstatisticas();
  ...
  if (isError || !stats) {
    return (
      <div className="space-y-6">
        <h1 className="text-3xl font-bold tracking-tight" style={{ color: COR }}>Lotomania · Resumo Estatístico</h1>
        <Card>
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card>
            ...
            <CardContent>
                  ...
                  {maisSorteadas.map((item, i) => (
                    <TableRow key={item.dezena} className="odd:bg-muted/40">
                      ...
                      <TableCell className="text-right">
                        {item.ultimoConcurso ? (
                          <Link
                            href={`/lotomania/resultado/${item.ultimoConcurso}`}
              ...
              <div className="mt-3 flex justify-end">
                <Link
                  href="/lotomania/tabela-de-dezenas?tab=mais"
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="mt-3 flex justify-end">
                <Link
                  href="/lotomania/tabela-de-dezenas?tab=menos"
                  className="text-sm font-semibold hover:underline"
                  style={{ color: COR }}
                >
                  Ver todas →
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="mt-3 flex justify-end">
                <Link
                  href="/lotomania/tabela-de-dezenas?tab=atrasadas"
                  className="text-sm font-semibold hover:underline"
                  style={{ color: COR }}
                >
                  Ver todas →
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                rows={stats.paresImpares
                  ...
                  .map(d => [
                    ...
                    totalSorteios > 0
                      ...
                      : "–",
                    d.ultimoConcurso
                      ? <Link href={`/lotomania/resultado/${d.ultimoConcurso}`} className="font-semibold hover:underline whitespace-nowrap" style={{ color: COR }}>Concurso {d.ultimoConcurso} →</Link>
                      : "–",
                  ])}
              />
            </CardContent>
          ...
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription>
                Quantas dezenas do sorteio caem na borda (moldura) ou no interior (retrato) do volante 10×10?
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="h-[200px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={stats.molduraRetrato ?? []} margin={{ top: 22, right: 8, left: -20, bottom: 0 }}>
                    ...
                    />
                    <Bar dataKey="sorteios" fill={COR} radius={[4, 4, 0, 0]}>
                      <LabelList dataKey="sorteios" position="top" style={{ fontSize: 11, fontWeight: "bold", fill: "#333" }} formatter={(v: number) => v.toLocaleString("pt-BR")} />
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
                  ...
                  <AccordionContent>
                    <div className="space-y-2 text-xs text-muted-foreground">
                      <div>
                        ...
                        {Array.from(MOLDURA_SET).sort(sortVolante).map(n => String(n).padStart(2, "0")).join(", ")}
                      </div>
                      <div>
                        <span className="font-semibold text-foreground">Retrato ({RETRATO_SET.size} dezenas):{" "}</span>
                        {Array.from(RETRATO_SET).sort(sortVolante).map(n => String(n).padStart(2, "0")).join(", ")}
                      </div>
                    </div>
                  </AccordionContent>
                </AccordionItem>
              ...
              <CardDescription>
                {stats.somaDezenas.menor && stats.somaDezenas.maior ? (
                  <>
                    ...
                    (
                    <Link
                      href={`/lotomania/resultado/${stats.somaDezenas.menor.concurso}`}
                    ...
                    (
                    <Link
                      href={`/lotomania/resultado/${stats.somaDezenas.maior.concurso}`}
                      className="font-semibold hover:underline"
                      style={{ color: COR }}
                    >
      ...
      {/* ── Seção 3: Distribuição na Grade 10×10 ── */}
      <section className="space-y-3">
        <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">
          Distribuição na Grade 10×10
        </h2>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription>
                Ocorrências acumuladas por linha do volante (L1 = 00–09, L2 = 10–19…)
              </CardDescription>
            </CardHeader>
            <CardContent>
                      ...
                      {Array.from({ length: 10 }, (_, i) => (
                        <div key={i}>
                          ...
                          {Array.from({ length: 10 }, (_, j) => {
                            // Linha i (0-based): dezenas de i*10+1 até i*10+10
                            // Última linha (i=9): 91–99, 00
                            const n = i * 10 + j + 1;
                            return n <= 99 ? String(n).padStart(2, "0") : "00";
                          }).join(", ")}
                        </div>
                      ))}
                    </div>
          ...
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription>
                Col. 0 = {"{"} 01,11,...,91 {"}"} … Col. 9 = {"{"} 10,20,...,90,00 {"}"}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="h-[220px]">
                  ...
                  >
                    ...
                    <Tooltip
                      ...
                      content={({ active, payload }) => {
                        ...
                        const dezenas = Array.from({ length: 10 }, (_, i) => d.coluna + i * 10)
                  ...
                  <AccordionContent>
                    <div className="space-y-1 text-xs text-muted-foreground">
                      {Array.from({ length: 10 }, (_, i) => i).map(col => {
                        // Coluna col (0-based): dezenas col+1, col+11, ..., col+91
                        // Última dezena da coluna 9 é "00" (posição do 100)
                        const dezenas = Array.from({ length: 10 }, (_, j) => {
                          const n = col + j * 10 + 1;
                          return n <= 99 ? String(n).padStart(2, "0") : "00";
                        }).join(", ");
                        return (
                ...
                <CardContent className="pt-5 space-y-5">
                  <div className="flex flex-wrap gap-x-8 gap-y-1 text-sm">
                    <div>
                      ...
                      <span className="font-semibold">{especial.quantidadeNaFaixa} dezenas entre 00 e 99</span>
                    </div>
                    <div>
