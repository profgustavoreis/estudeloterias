...
import { useGerarJogoLotomania } from "@workspace/api-client-react";
import { Sparkles, Printer, RefreshCw } from "lucide-react";
import { PageSEO } from "@/components/seo/PageSEO";

const COR = "#f8901c";
const COR_HOVER = "#d97900";
...
export default function LotomaniaGerador() {
  ...
  return (
    <div className="space-y-6">
      <PageSEO
        title="Gerador de Jogos da Lotomania"
        description="Gere apostas aleatórias para a Lotomania. Surpresinha inteligente e gratuita: cada jogo tem 50 dezenas de 00 a 99."
        canonical="/lotomania/gerador"
      />
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight" style={{ color: COR }}>Lotomania · Gerador de Jogos</h1>
          <p className="text-muted-foreground mt-1">Crie jogos aleatórios de 50 dezenas para a Lotomania.</p>
        </div>
      </div>
        ...
        <Card className="lg:col-span-1 h-fit">
          ...
          <CardContent className="space-y-8">
            ...
            <div className="p-4 bg-muted/50 rounded-lg border border-border space-y-1">
              <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Aposta da Lotomania</div>
              <p className="text-sm text-muted-foreground">
                Cada jogo é uma aposta de <strong>50 dezenas</strong> (00 a 99).
              </p>
              <p className="text-sm text-muted-foreground">
                  ...
                  <Card key={i}>
                    ...
                    <CardContent className="p-4">
                      <div className="flex flex-wrap gap-1">
                        {jogo.sort(sortVolante).map(num => (
                          <LotteryBall key={num} number={num} size="sm" color={COR} />
                        ))}
                      </div>
                    </CardContent>
                  </Card>
