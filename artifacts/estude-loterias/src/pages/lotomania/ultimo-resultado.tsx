...
import {
  useGetLotomaniaUltimoResultado,
  useGetLotomaniaResultadoConcurso,
...
function isMoldura(n: number): boolean {
  // Na Lotomania, "00" (n=0) ocupa a última posição do volante (após 99).
  // Convertemos para posição 0-based: "00"→99, "01"→0, ..., "99"→98
  const pos = n === 0 ? 99 : n - 1;
...
function DezenasCard({ resultado }: { resultado: ResultadoLotomania }) {
  return (
    ...
    <Card className="flex flex-col h-full">
      ...
      <CardContent className="space-y-4 flex-1">
        <div>
          ...
          {moldura.length > 0 ? <Balls dezenas={moldura} size="sm" /> : <span className="text-sm text-muted-foreground">Nenhum</span>}
        </div>
        <div>
          <SectionLabel>Retrato: {retrato.length}</SectionLabel>
          {retrato.length > 0 ? <Balls dezenas={retrato} size="sm" /> : <span className="text-sm text-muted-foreground">Nenhum</span>}
        </div>
      </CardContent>
    </Card>
  );
...
function RepeticoesCard({ concurso, dezenas }: { concurso: number; dezenas: string[] }) {
  const { data: prev } = useGetLotomaniaResultadoConcurso(concurso - 1);
...
function RateioCard({ resultado }: { resultado: ResultadoLotomania }) {
  return (
    <Card>
...
function ResultadoView({ resultado, latestConcurso }: { resultado: ResultadoLotomania; latestConcurso: number }) {
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-3xl font-bold tracking-tight" style={{ color: BRAND }}>
            Lotomania · Concurso {resultado.concurso}
          </h1>
          <p className="text-muted-foreground mt-1 text-sm">Sorteio realizado dia {formatDateShort(resultado.data)}</p>
        </div>
        <ConcursoNavigator concurso={resultado.concurso} isLatest={isLatest} latestConcurso={latestConcurso} basePath="/lotomania/resultado" color={BRAND} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="flex items-start justify-center">
          <AdUnit slot="7788990011" format="rectangle" className="w-full max-w-sm" />
        </div>
      </div>
...
function LatestResultado() {
  const { data, isLoading, isError } = useGetLotomaniaUltimoResultado();
...
function ConcursoResultado({ concurso }: { concurso: number }) {
  const { data, isLoading, isError } = useGetLotomaniaResultadoConcurso(concurso);
  const { data: latest } = useGetLotomaniaUltimoResultado();
  if (isLoading) return <LoadingSkeleton />;
...
export default function LotomaniaUltimoResultado() {
