...
// Ordenação por dezena que coloca "00" na última posição, após o 99
const dezenaSortValue = (d: string) => d === "00" ? 100 : parseInt(d, 10);
...
function UltimaVezLink({ concurso }: { concurso: number | null }) {
  ...
  return (
    <Link
      href={`/lotomania/resultado/${concurso}`}
...
export default function LotomaniaEstatisticasTabela() {
  ...
  const rows = (() => {
    if (tab === "atrasadas") {
      if (sortAtr === "dezena")
        return base.sort((a, b) => dezenaSortValue(a.dezena) - dezenaSortValue(b.dezena));
