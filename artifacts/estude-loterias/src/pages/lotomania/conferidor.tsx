...
import { useGetLotomaniaUltimoResultado, useGetLotomaniaResultadoConcurso } from "@workspace/api-client-react";
import type { ResultadoLotomania } from "@workspace/api-client-react";
...
// Faixas premiadas da Lotomania: 20→faixa1, 19→faixa2, 18→faixa3, 17→faixa4, 16→faixa5, 15→faixa6, 0→faixa7
const FAIXA_FOR_ACERTOS: Record<number, number> = {
  20: 1, 19: 2, 18: 3, 17: 4, 16: 5, 15: 6, 0: 7,
};
...
function ResultadoCard({ resultado, selecionadas }: { resultado: ResultadoLotomania; selecionadas: Set<number> }) {
  ...
  const faixaLabel = (() => {
    if (K === 0) return "0 acertos (nenhum número)";
    return `${K} acerto${K > 1 ? "s" : ""}`;
  })();
  ...
  return (
    <Card className="border-t-4" style={{ borderTopColor: COR }}>
      <CardHeader className="pb-3">
        ...
        <CardDescription>
          Aposta de 50 dezenas • {formatCurrency(3.0)}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {K === 0 ? <span className="text-sm text-muted-foreground">—</span> : (
                Array.from(acertadas).sort(sortVolante).map(d => (
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {TOTAL_DEZENAS - K === 0 ? <span className="text-sm text-muted-foreground">—</span> : (
                Array.from(selecionadas).sort(sortVolante).filter(d => !acertadas.has(d)).map(d => (
        ...
        {temPremio && premio ? (
          ...
          <p className="text-sm text-muted-foreground">
            Nenhuma faixa de premiação atingida. Na Lotomania, os prêmios são para 20, 19, 18, 17, 16, 15 ou 0 acertos.
          </p>
        )}
      </CardContent>
    </Card>
...
export default function LotomaniaConferidor() {
  ...
  const ultimo = useGetLotomaniaUltimoResultado();
  ...
  const concursoResult = useGetLotomaniaResultadoConcurso(concursoQuery ?? 0);
  ...
  return (
    <div className="space-y-6">
      <PageSEO
        title="Conferidor de Apostas da Lotomania"
        description="Confira se sua aposta da Lotomania ganhou! Escolha suas 50 dezenas, selecione o concurso e veja seus acertos e o prêmio correspondente."
        canonical="/lotomania/conferidor"
      />
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight" style={{ color: COR }}>Lotomania · Conferidor de Apostas</h1>
          <p className="text-muted-foreground mt-1">Selecione 50 dezenas e confira se sua aposta ganhou.</p>
        </div>
      </div>
              ...
              {[...Array.from({ length: 99 }, (_, i) => i + 1), 0].map(n => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "relative aspect-square rounded text-[10px] font-bold transition-all duration-150 select-none",
                      hit         ? "text-white shadow-sm ring-1 scale-105"
                      : miss      ? "bg-destructive text-white scale-105"
                      : sel       ? "text-white shadow-sm ring-1 scale-105"
                      : count >= TOTAL_DEZENAS ? "bg-muted/30 text-muted-foreground/40 cursor-not-allowed"
            ...
            <CardContent className="pt-5 text-sm text-muted-foreground">
              ...
              {infoAberta && (
                <ol className="mt-3 space-y-1.5 list-decimal list-inside">
                  ...
                  <li>Acertos aparecem em laranja, erros em vermelho e dezenas sorteadas não marcadas em laranja claro.</li>
                </ol>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="pt-5 space-y-4">
              ...
              {!podeConferir && count > 0 && (
                <p className="text-sm text-amber-600 text-center">Selecione mais {TOTAL_DEZENAS - count} dezena{TOTAL_DEZENAS - count > 1 ? "s" : ""} para conferir.</p>
              )}
            </CardContent>
          </Card>
