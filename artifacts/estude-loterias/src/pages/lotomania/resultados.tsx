...
import { useGetLotomaniaResultados } from "@workspace/api-client-react";
...
export default function LotomaniaResultadosAnteriores() {
  const [page, setPage] = useState(1);
  const [ano, setAno] = useState<number | null>(null);
  const [ordem, setOrdem] = useState<"asc" | "desc">("desc");

  const { data, isLoading, isError } = useGetLotomaniaResultados({ page, limit: 20, ano, ordem });
  ...
  return (
    <div className="space-y-6">
      <PageSEO
        title="Todos os Resultados da Lotomania"
        description="Consulte o histórico completo de todos os resultados da Lotomania. Filtre por ano, veja dezenas sorteadas e prêmios de cada concurso."
        canonical="/lotomania/resultados"
      />
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight" style={{ color: COR }}>Lotomania · Resultados Anteriores</h1>
            <p className="text-muted-foreground mt-1">Busque e analise os concursos passados da Lotomania.</p>
          </div>
        </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/50 hover:bg-muted/50">
                  ...
                  <TableHead className="text-right">Prêmio 20 acertos</TableHead>
                ...
                {isLoading ? (
                  Array.from({ length: 10 }).map((_, i) => (
                    <TableRow key={i}>
                      ...
                      <TableCell><div className="flex justify-center gap-0.5 flex-wrap">{Array.from({ length: 20 }).map((_, j) => <Skeleton key={j} className="h-6 w-6 rounded-full" />)}</div></TableCell>
                  ...
                  data.resultados.map(res => (
                    <TableRow key={res.concurso}>
                      ...
                      <TableCell className="text-right">
                        <Link href={`/lotomania/resultado/${res.concurso}`} className="text-sm font-semibold hover:underline whitespace-nowrap" style={{ color: COR }}>
                          Ver detalhes →
                        </Link>
                      </TableCell>
                    </TableRow>
