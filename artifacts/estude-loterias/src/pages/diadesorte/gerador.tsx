...
import { useGerarJogoDiadesorte } from "@workspace/api-client-react";
import { Sparkles, Printer, RefreshCw } from "lucide-react";
import { PageSEO } from "@/components/seo/PageSEO";

const COR = "#cb852b";
const COR_HOVER = "#a66d1e";
const PRECO_APOSTA = 2.5;

export default function DiaDeSorteGerador() {
  const [qtdJogos, setQtdJogos] = useState(1);
  const [qtdDezenas, setQtdDezenas] = useState(7);
  const [resultado, setResultado] = useState<{ jogos: number[][]; custo: number } | null>(null);

  const gerarJogo = useGerarJogoDiadesorte({
  ...
  return (
    <div className="space-y-6">
      <PageSEO
        title="Gerador de Jogos do Dia de Sorte"
        description="Gere apostas aleatórias para o Dia de Sorte. Surpresinha inteligente e gratuita: cada jogo tem de 7 a 15 dezenas de 01 a 31."
        canonical="/diadesorte/gerador"
      />
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight" style={{ color: COR }}>Dia de Sorte · Gerador de Jogos</h1>
          <p className="text-muted-foreground mt-1">Crie jogos aleatórios de 7 a 15 dezenas para o Dia de Sorte.</p>
        </div>
      </div>
        ...
        <Card className="lg:col-span-1 h-fit">
          ...
          <CardContent className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              <Slider value={[qtdDezenas]} min={7} max={15} step={1} onValueChange={v => setQtdDezenas(v[0])} />
              <div className="text-xs text-muted-foreground">Aposta mínima: 7 dezenas (R$ 2,50). Máxima: 15 dezenas.</div>
            </div>

            <div className="p-4 bg-muted/50 rounded-lg border border-border space-y-1">
              <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Aposta do Dia de Sorte</div>
              <p className="text-sm text-muted-foreground">
                Cada jogo é uma aposta de <strong>{qtdDezenas} dezenas</strong> (01 a 31).
              </p>
              <p className="text-sm text-muted-foreground">
