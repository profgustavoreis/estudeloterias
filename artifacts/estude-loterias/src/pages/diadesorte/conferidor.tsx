...
import { useGetDiadesorteUltimoResultado, useGetDiadesorteResultadoConcurso } from "@workspace/api-client-react";
...
function ResultadoCard({ resultado, selecionadas }: { resultado: ResultadoDiadesorte; selecionadas: Set<number> }) {
  ...
  return (
    <Card className="border-t-4" style={{ borderTopColor: COR }}>
      <CardHeader className="pb-3">
        ...
        <CardDescription>Aposta de 7 dezenas • {formatCurrency(2.5)}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        {temPremio && premio ? (
          ...
          <p className="text-sm text-muted-foreground">Nenhuma faixa de premiação atingida. No Dia de Sorte, os prêmios são para 7, 6, 5, 4 ou 3 acertos.</p>
        )}
      </CardContent>
    </Card>
  );
...
export default function DiaDeSorteConferidor() {
  ...
  const ultimo = useGetDiadesorteUltimoResultado();
  ...
  const concursoResult = useGetDiadesorteResultadoConcurso(concursoQuery ?? 0);
  ...
  return (
    <div className="space-y-6">
      <PageSEO
        title="Conferidor de Apostas do Dia de Sorte"
        description="Confira se sua aposta do Dia de Sorte ganhou! Escolha suas 7 dezenas, selecione o concurso e veja seus acertos e o prêmio correspondente."
        canonical="/diadesorte/conferidor"
      />
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight" style={{ color: COR }}>Dia de Sorte · Conferidor de Apostas</h1>
          <p className="text-muted-foreground mt-1">Selecione 7 dezenas e confira se sua aposta ganhou.</p>
        </div>
      </div>
