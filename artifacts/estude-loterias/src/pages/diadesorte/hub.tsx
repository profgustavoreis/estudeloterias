import { useGetDiadesorteUltimoResultado } from "@workspace/api-client-react";
...
export default function DiaDeSorteHub() {
  const { data: resultado, isLoading } = useGetDiadesorteUltimoResultado();

  const quickLinks = [
    { href: `/diadesorte/resultado/${resultado?.concurso ?? ""}`, label: "Último Resultado", icon: Dices, desc: "Detalhes do último sorteio" },
    { href: "/diadesorte/resultados", label: "Resultados Anteriores", icon: List, desc: "Busque por sorteios passados" },
    { href: "/diadesorte/resumo-estatistico", label: "Resumo Estatístico", icon: BarChart3, desc: "Frequência de dezenas e atrasos" },
    { href: "/diadesorte/tabela-de-dezenas",  label: "Tabela de Dezenas",  icon: TableIcon, desc: "Ranking detalhado de todas as dezenas" },
    { href: "/diadesorte/gerador", label: "Gerador de Jogos", icon: Sparkles, desc: "Gere jogos inteligentes" },
    { href: "/diadesorte/simulador", label: "Simulador Histórico", icon: FlaskConical, desc: "Confira sua aposta no histórico" },
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight" style={{ color: COR }}>Dia de Sorte · Painel Principal</h1>
            <p className="text-muted-foreground mt-1">Escolha de 7 a 15 números entre 01 e 31 e concorra a prêmios toda semana!</p>
          </div>
        </div>
        <ConcursoNavigator
        ...
        <Card className="lg:col-span-2 border-t-4" style={{ borderTopColor: COR }}>
          <CardHeader>
            <div className="flex justify-between items-start">
              ...
              <Link href={`/diadesorte/resultado/${resultado.concurso}`}>
          ...
          <CardContent className="space-y-6">
            ...
            {resultado.mesSorte && (
              <div className="flex items-center justify-center gap-2">
                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Mês da Sorte:</span>
                <span className="text-sm font-bold px-3 py-1 rounded-full text-white" style={{ backgroundColor: COR }}>{resultado.mesSorte}</span>
              </div>
            )}
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">
                No Dia de Sorte você escolhe de 7 a 15 números entre 01 e 31 e um Mês da Sorte.
                São sorteados 7 números e um Mês da Sorte por concurso. Você ganha acertando de 3 a 7 números
                ou o Mês da Sorte. O Dia de Sorte não possui sorteio especial.
              </p>
            </div>
          </CardContent>
        </Card>
