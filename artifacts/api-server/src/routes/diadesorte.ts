...
function toResultado(row: typeof lotteryResultsTable.$inferSelect) {
  ...
  return {
    ...
    mesSorte: meta?.nomeEspecial ?? null,
  };
}

router.get("/diadesorte/resultado/ultimo", async (req, res) => {
  try {
    ...
    req.log.error({ err }, "Failed to get diadesorte ultimo resultado");
    res.status(500).json({ error: "Erro ao buscar resultado" });
  }
});

router.get("/diadesorte/resultados", async (req, res) => {
  try {
    ...
    req.log.error({ err }, "Failed to get diadesorte resultados");
    res.status(500).json({ error: "Erro ao buscar resultados" });
  }
});

router.get("/diadesorte/resultados/:concurso", async (req, res) => {
  ...
  try {
    ...
    req.log.error({ err, concurso }, "Failed to get diadesorte concurso");
    res.status(404).json({ error: "Concurso não encontrado" });
  }
});

router.get("/diadesorte/estatisticas", async (req, res) => {
  try {
    const rows = await db
      .select()
      .from(lotteryResultsTable)
    ...
    // Dia de Sorte: 7 dezenas de 01 a 31 (min=28, max=196)
    const SOMA_BUCKETS = [
      { min: 28, max: 49,  faixa: "28–49"  },
      { min: 50, max: 79,  faixa: "50–79"  },
      { min: 80, max: 109, faixa: "80–109" },
      { min: 110, max: 139, faixa: "110–139" },
      { min: 140, max: 169, faixa: "140–169" },
      { min: 170, max: 196, faixa: "170–196" },
    ];
    ...
    // Grid 4×10: moldura = border (row1 + row4 + left/right of rows 2–3)
    const MOLDURA = new Set<number>([
      ...Array.from({ length: 10 }, (_, i) => i + 1),     // row 1: 01–10
      ...Array.from({ length: 1 }, (_, i) => i + 31),      // row 4: 31
      11, 20, 21, 30,                                        // left/right of rows 2–3
    ]);
    ...
    const freqLinha = Array(4).fill(0);    // 4 rows
    ...
    const FAIXAS_LINHAS = ["01–10","11–20","21–30","31"];
    ...
    req.log.error({ err }, "Failed to compute diadesorte estatisticas");
    res.status(500).json({ error: "Erro ao calcular estatísticas" });
  }
});

router.get("/diadesorte/resumo", async (req, res) => {
  try {
    const [maxRow] = await db
    ...
    req.log.error({ err }, "Failed to get diadesorte resumo");
    res.status(500).json({ error: "Erro ao buscar resumo" });
  }
});

router.post("/diadesorte/simulador", async (req, res) => {
  try {
    ...
    req.log.error({ err }, "Failed to simulate diadesorte");
    res.status(500).json({ error: "Erro ao simular" });
  }
});

router.post("/diadesorte/gerador", async (req, res) => {
  try {
    const { quantidadeJogos = 1, quantidadeDezenas = 7 } = req.body ?? {};

    const qtdJogos = Math.min(20, Math.max(1, parseInt(String(quantidadeJogos), 10)));
    const qtdDezenas = Math.min(15, Math.max(7, parseInt(String(quantidadeDezenas), 10)));

    const custoPorJogo = 2.5;
    ...
    req.log.error({ err }, "Failed to generate diadesorte jogo");
